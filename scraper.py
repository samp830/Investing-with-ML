...
# print (script_dir)
# rel_path = "daily_prices/"
# abs_file_path = os.path.join(script_dir, re)

def th(url):
    myfile = open("/Users/sparajuli/Desktop/UCB/fun/Stock_SVM/daily_prices/" + symbol + ".txt", "w+")
    myfile.close()
    rawdata = urllib.request.urlopen(url).read()
    jsonData = json.loads(rawdata.decode('utf-8'))
    myfile = open("/Users/sparajuli/Desktop/UCB/fun/Stock_SVM/daily_prices/" + symbol + ".txt", "a")
    for element in jsonData[0]['price']:
        myfile.write(str(symbol) + "," + str(element["dateTime"]) + "," + str(element["value"]) + "\n")
    myfile.close()
